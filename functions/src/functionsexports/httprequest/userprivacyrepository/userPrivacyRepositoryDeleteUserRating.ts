...
export const userPrivacyRepositoryDeleteUserRating = functions.region('europe-west1').https.onCall(async (data: any, context: any) => {
    ...
    try {
        ...
        const userOfRating:string = parsedSnapshot.userOfRating;

        if (userOfRating === context.auth.uid)
            throw new ValidationException("Δεν επιτρέπεται αυτή η κίνηση");


        await userPrivacyRepository.deleteUserRating(userOfRating, context.auth.uid);
